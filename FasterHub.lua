...
local ESPColors = {
    {name = "Xanh lá (Gốc)", val = Color3.fromRGB(0,255,0)}, {name = "Đỏ", val = Color3.fromRGB(255,0,0)},
...
-- Tốc độ & Nhảy (Fix lỗi Off vẫn chạy)
...
-- Loop chính để giữ Speed/Jump không bị game đè
    ...
    if states.WallClimb then
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if ray and not ray.Instance:IsDescendantOf(char) then
                hrp.Velocity = Vector3.new(hrp.Velocity.X, 40, hrp.Velocity.Z) -- Đẩy nhân vật lên
            end
        end
    end
end)
...
-- ==========================================
-- 6. HỆ THỐNG FLY CHO ĐIỆN THOẠI (FIX VỊ TRÍ)
-- ==========================================
...
-- ĐÃ SỬA VỊ TRÍ CAO LÊN ĐỂ KHÔNG VƯỚNG NÚT NHẢY
...
CreateSelector(PageSet, "Màu Đường Mòn", TrailColors, function(val)
    ...
    if trailObject then
        if val == "Rainbow" then
            -- Loop RenderStepped phía trên sẽ lo việc đổi màu
        else
