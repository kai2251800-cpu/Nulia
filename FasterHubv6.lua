...
if CoreGui:FindFirstChild("FasterHubV6") then
    CoreGui:FindFirstChild("FasterHubV6"):Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "FasterHubV6"
...
BlockScreen.BackgroundTransparency = 0.2
...
bindable.OnInvoke = function(res)
    if res == "Có✅" then StartLoading() end
end
...
task.spawn(function()
    pcall(function()
        StarterGui:SetCore("SendNotification", {
            Title = "Faster Hub V6",
            Text = "Bạn có muốn kích hoạt script không?",
            Duration = 5,
            Button1 = "Có✅",
            Button2 = "Không ❌",
            Callback = bindable
    ...
    task.wait(6)
    if not hasResponded then StartLoading() end -- Fallback nếu ĐT lỗi nút
end)
...
CheckBtn.MouseButton1Click:Connect(function()
    if KeyInput.Text == "FasterHub_SuperVip" then
        ...
        for i = 0, 100 do LoadPercent.Text = i .. "%" LoadBarFill.Size = UDim2.new(i/100, 0, 1, 0) task.wait(0.015) end
...
-- ==========================================
-- 2. GIAO DIỆN HUB CHÍNH V6
-- ==========================================
local MainFrame = Instance.new("Frame", ScreenGui)
MainFrame.Size = UDim2.new(0, 430, 0, 480)
...
Title.Text = "FASTER HUB V6 [GOD MODE & MM2]"
...
TabScroll.CanvasSize = UDim2.new(2.5, 0, 0, 0)
...
local infJumpBtn, _ = CreateToggle(Pages.Home, "Infi Jump (Nhảy Vô Hạn)")
local godBtn, _ = CreateToggle(Pages.Home, "God Mode (Bất Tử)")
...
ZLbl.Text = "Povzoom (Chỉnh góc nhìn)"
ZLbl.TextColor3 = Color3.new(1,1,1)
ZLbl.Font = Enum.Font.Gotham
ZLbl.TextSize = 13
local btnMin = Instance.new("TextButton", ZoomFrame)
btnMin.Size = UDim2.new(0, 30, 0, 30) btnMin.Position = UDim2.new(0.7, 0, 0.5, -15) btnMin.BackgroundColor3 = Color3.fromRGB(30,30,30) btnMin.Text = "-" btnMin.TextColor3 = Color3.new(1,1,1) Instance.new("UICorner", btnMin)
local btnPls = Instance.new("TextButton", ZoomFrame)
btnPls.Size = UDim2.new(0, 30, 0, 30) btnPls.Position = UDim2.new(0.85, 0, 0.5, -15) btnPls.BackgroundColor3 = Color3.fromRGB(30,30,30) btnPls.Text = "+" btnPls.TextColor3 = Color3.new(1,1,1) Instance.new("UICorner", btnPls)
...
RegisterToggle(infJumpBtn, "InfJump")
RegisterToggle(godBtn, "GodMode")
...
local distBtn, _ = CreateToggle(Pages.ESP_Fly, "Khoảng Cách (Hiện M)")
...
local function UpdateESP()
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character then
            ...
            -- Distance ESP
            local distUI = plr.Character:FindFirstChild("DistUI")
            if states.Distance then
                if not distUI and plr.Character:FindFirstChild("Head") then
                    distUI = Instance.new("BillboardGui", plr.Character) distUI.Name = "DistUI"
                    distUI.Size = UDim2.new(0, 100, 0, 40) distUI.StudsOffset = Vector3.new(0, 3, 0) distUI.AlwaysOnTop = true
                    local txt = Instance.new("TextLabel", distUI) txt.Size = UDim2.new(1,0,1,0) txt.BackgroundTransparency = 1 txt.TextColor3 = Color3.new(1,1,1) txt.TextStrokeTransparency = 0 txt.Font = Enum.Font.GothamBold txt.TextScaled = true
                end
            else
                if distUI then distUI:Destroy() end
            end
        end
    end
end
RegisterToggle(espBtn, "ESP", function() UpdateESP() end)
RegisterToggle(distBtn, "Distance", function() UpdateESP() end)
...
-- TAB DỊCH CHUYỂN (TP PLAYER & TeleOld)
local teleOldBtn, _ = CreateToggle(Pages.TP, "TeleOld (Biểu tượng ❗)")
local TeleOldIcon = Instance.new("TextButton", ScreenGui)
TeleOldIcon.Size = UDim2.new(0, 50, 0, 50)
TeleOldIcon.Position = UDim2.new(1, -70, 0.3, 0)
TeleOldIcon.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
TeleOldIcon.Text = "❗"
TeleOldIcon.TextSize = 25
TeleOldIcon.Visible = false
Instance.new("UICorner", TeleOldIcon).CornerRadius = UDim.new(1,0)

RegisterToggle(teleOldBtn, "TeleOld", function(v) TeleOldIcon.Visible = v end)
TeleOldIcon.MouseButton1Click:Connect(function()
    local spawnLoc = workspace:FindFirstChildOfClass("SpawnLocation")
    if spawnLoc and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        LocalPlayer.Character.HumanoidRootPart.CFrame = spawnLoc.CFrame + Vector3.new(0, 5, 0)
    end
end)

local function RefreshTPList()
    for _, c in pairs(Pages.TP:GetChildren()) do if c:IsA("TextButton") and c.Name ~= "RefreshBtn" then c:Destroy() end end
    local RefreshBtn = CreateButton(Pages.TP, "🔄 Làm Mới Danh Sách", function() RefreshTPList() end) RefreshBtn.Name = "RefreshBtn"
    
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            CreateButton(Pages.TP, "👤 " .. plr.Name, function()
    ...
    Pages.TP.CanvasSize = UDim2.new(0, 0, 0, #Players:GetPlayers() * 45 + 100)
end
RefreshTPList()
...
local mm2AimBtn, _ = CreateToggle(Pages.MM2, "Aimbot (Giết Sát Nhân)")

local function getMM2RoleColor(plr)
    local bp = plr:FindFirstChild("Backpack")
    local char = plr.Character
    if (bp and bp:FindFirstChild("Knife")) or (char and char:FindFirstChild("Knife")) then return Color3.fromRGB(255, 0, 0) end
    if (bp and (bp:FindFirstChild("Gun") or bp:FindFirstChild("Revolver"))) or (char and (char:FindFirstChild("Gun") or char:FindFirstChild("Revolver"))) then return Color3.fromRGB(0, 150, 255) end
    return Color3.fromRGB(0, 255, 0)
end

RegisterToggle(mm2ESPBtn, "MM2_ESP", function(v)
    if not v then
        for _, plr in pairs(Players:GetPlayers()) do
            if plr.Character and plr.Character:FindFirstChild("MM2_HL") then plr.Character.MM2_HL:Destroy() end
        end
    end
end)
...
local function getMurderer()
    for _, plr in pairs(Players:GetPlayers()) do
        local bp = plr:FindFirstChild("Backpack")
        local char = plr.Character
        if plr ~= LocalPlayer and ((bp and bp:FindFirstChild("Knife")) or (char and char:FindFirstChild("Knife"))) then
            return plr
        end
    end
    return nil
...
RegisterToggle(mm2AimBtn, "MM2_Aim")
-- Silent Aim hook for MM2 (Chuyển hướng viên đạn vào sát nhân)
local oldIndex
oldIndex = hookmetamethod(game, "__index", function(self, key)
    if not checkcaller() and states.MM2_Aim and self == LocalPlayer:GetMouse() then
        if key == "Hit" or key == "Target" then
            local murd = getMurderer()
            if murd and murd.Character and murd.Character:FindFirstChild("HumanoidRootPart") then
                if key == "Hit" then return murd.Character.HumanoidRootPart.CFrame end
                if key == "Target" then return murd.Character.HumanoidRootPart end
            end
        end
    end
...
RegisterToggle(ragdollBtn, "Ragdoll", function(v)
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.PlatformStand = v end
end)
...
RegisterToggle(noHeadFootBtn, "NoHeadFoot", function(v)
    ...
    if char then
        ...
        for _, p in pairs(parts) do if char:FindFirstChild(p) then char[p].Transparency = v and 1 or 0 end end
...
local BgColors = {
    {name="Đen",val=Color3.fromRGB(25,25,25)}, {name="Đỏ",val=Color3.fromRGB(40,15,15)}, {name="Xanh",val=Color3.fromRGB(15,20,40)},
    {name="Trắng Ngà",val=Color3.fromRGB(240,240,240)}, {name="Hồng",val=Color3.fromRGB(50,20,30)}, {name="Lam Đậm",val=Color3.fromRGB(10,30,40)},
    {name="Cam",val=Color3.fromRGB(40,25,10)}, {name="Nâu",val=Color3.fromRGB(30,20,10)}
}
local ESPColors = {
    {name="Xanh Lá",val=Color3.new(0,1,0)}, {name="Đỏ",val=Color3.new(1,0,0)}, {name="Trắng",val=Color3.new(1,1,1)},
    {name="Hồng",val=Color3.fromRGB(255,105,180)}, {name="Tím",val=Color3.fromRGB(128,0,128)}, {name="Vàng",val=Color3.new(1,1,0)},
    {name="Cam",val=Color3.fromRGB(255,165,0)}, {name="Xanh Dương",val=Color3.new(0,0,1)}, {name="Đen",val=Color3.new(0,0,0)},
    {name="Nâu",val=Color3.fromRGB(139,69,19)}, {name="Cyan",val=Color3.fromRGB(0,255,255)}, {name="Lime",val=Color3.fromRGB(50,205,50)},
    {name="Xám",val=Color3.fromRGB(128,128,128)}
}

CreateSelector(Pages.Set, "Màu Nền Hub", BgColors, function(v) MainFrame.BackgroundColor3 = v end)
CreateSelector(Pages.Set, "Màu Định Vị", ESPColors, function(v) currentESPColor = v UpdateESP() end)
CreateSelector(Pages.Set, "Màu Đường Mòn", ESPColors, function(v) currentTrailColor = v if trailObj then trailObj.Color = ColorSequence.new(v) end end)

Pages.Home.CanvasSize = UDim2.new(0, 0, 0, 450)
Pages.ESP_Fly.CanvasSize = UDim2.new(0, 0, 0, 250)
Pages.TP.CanvasSize = UDim2.new(0, 0, 0, 300)
Pages.Troll.CanvasSize = UDim2.new(0, 0, 0, 300)
Pages.MM2.CanvasSize = UDim2.new(0, 0, 0, 200)
...
Pages.Set.CanvasSize = UDim2.new(0, 0, 0, 300)
...
RunService.Stepped:Connect(function(dt)
    ...
    -- God Mode
    if states.GodMode and hum then
    ...
    if states.RainbowSkin then
        ...
        for _, p in pairs(char:GetDescendants()) do if p:IsA("BasePart") or p:IsA("MeshPart") then p.Color = rc end end
    end
    ...
    if states.Distance then
        for _, plr in pairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("Head") and hrp then
                local distUI = plr.Character:FindFirstChild("DistUI")
                if distUI and distUI:FindFirstChildOfClass("TextLabel") then
                    local dist = math.floor((hrp.Position - plr.Character.Head.Position).Magnitude)
                    distUI:FindFirstChildOfClass("TextLabel").Text = dist .. " M"
                end
            end
        end
    end
    ...
    if states.MM2_ESP then
        for _, plr in pairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer and plr.Character then
                local hl = plr.Character:FindFirstChild("MM2_HL")
                if not hl then hl = Instance.new("Highlight", plr.Character) hl.Name = "MM2_HL" hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop end
                hl.FillColor = getMM2RoleColor(plr)
            end
        end
    end
end)
