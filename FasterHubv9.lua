...
if CoreGui:FindFirstChild("FasterHubV9") then
    CoreGui:FindFirstChild("FasterHubV9"):Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "FasterHubV9"
...
-- ==========================================
-- 1. CUTSCENE INTRO & KEY (SỬA LỖI BẢNG ĐEN)
-- ==========================================
...
local IntroLabel = Instance.new("TextLabel", BlockScreen)
IntroLabel.Size = UDim2.new(1, 0, 0.2, 0)
IntroLabel.Position = UDim2.new(0, 0, 0.4, 0)
IntroLabel.BackgroundTransparency = 1
IntroLabel.Text = "Faster Hub Update 9"
IntroLabel.TextColor3 = Color3.new(0, 1, 1)
IntroLabel.Font = Enum.Font.GothamBold
IntroLabel.TextScaled = true
IntroLabel.ZIndex = 1000
...
KeyTitle.Text = "VUI LÒNG NHẬP KEY V9:"
...
GetKeyBtn.MouseButton1Click:Connect(function() 
    ...
    GetKeyBtn.Text = "Đã Copy!" task.wait(1) GetKeyBtn.Text = "Lấy link 🔑"
end)
CheckBtn.MouseButton1Click:Connect(function()
    if KeyInput.Text == "FasterHub_SuperVip" then
        ...
        CheckBtn.Text = "Key Sai!" task.wait(1) CheckBtn.Text = "Kiểm tra Key"
    end
end)

-- ==========================================
-- HỆ THỐNG XYZ THỜI GIAN THỰC
-- ==========================================
local XYZLabel = Instance.new("TextLabel", ScreenGui)
XYZLabel.Size = UDim2.new(0, 200, 0, 30) XYZLabel.Position = UDim2.new(1, -210, 0, 10)
XYZLabel.BackgroundTransparency = 0.5 XYZLabel.BackgroundColor3 = Color3.fromRGB(0,0,0)
XYZLabel.TextColor3 = Color3.new(0, 1, 0) XYZLabel.Font = Enum.Font.GothamBold XYZLabel.TextSize = 12 XYZLabel.Visible = false
...
MainFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 25) MainFrame.Visible = false MainFrame.Active = true MainFrame.Draggable = true
Instance.new("UICorner", MainFrame)

ToggleButton.MouseButton1Click:Connect(function() MainFrame.Visible = not MainFrame.Visible end)

local Title = Instance.new("TextLabel", MainFrame)
Title.Size = UDim2.new(1, 0, 0, 35) Title.BackgroundTransparency = 1 Title.Text = "FASTER HUB V9 [ULTIMATE]" Title.TextColor3 = Color3.new(1,1,1) Title.Font = Enum.Font.GothamBold Title.TextSize = 14

local TabScroll = Instance.new("ScrollingFrame", MainFrame)
TabScroll.Size = UDim2.new(1, 0, 0, 35) TabScroll.Position = UDim2.new(0, 0, 0, 35) TabScroll.BackgroundTransparency = 1 TabScroll.CanvasSize = UDim2.new(4.5, 0, 0, 0) TabScroll.ScrollBarThickness = 0
local TabListLayout = Instance.new("UIListLayout", TabScroll) TabListLayout.FillDirection = Enum.FillDirection.Horizontal TabListLayout.Padding = UDim.new(0, 5)

local function CreateTabBtn(name)
    local btn = Instance.new("TextButton", TabScroll) btn.Size = UDim2.new(0, 100, 1, 0) btn.BackgroundTransparency = 1 btn.Text = name btn.TextColor3 = Color3.fromRGB(150, 150, 150) btn.Font = Enum.Font.GothamBold btn.TextSize = 11 return btn
end

local Tabs = {
    Info = CreateTabBtn("THÔNG TIN"), Home = CreateTabBtn("CHỨC NĂNG"), ESP_Fly = CreateTabBtn("ĐỊNH VỊ & BAY"), TP = CreateTabBtn("DỊCH CHUYỂN"), Troll = CreateTabBtn("TROLL & BẢO VỆ"),
    Aura = CreateTabBtn("AURA & FX"), Skybox = CreateTabBtn("SKYBOX"), Anim = CreateTabBtn("ANIM & SHADER"), MM2 = CreateTabBtn("MM2 HACK"), WithGame = CreateTabBtn("WITH GAME?"), Set = CreateTabBtn("CÀI ĐẶT")
}
...
for k, _ in pairs(Tabs) do
    local page = Instance.new("ScrollingFrame", MainFrame) page.Size = UDim2.new(1, -20, 1, -85) page.Position = UDim2.new(0, 10, 0, 75) page.BackgroundTransparency = 1 page.ScrollBarThickness = 4 page.Visible = false
    local layout = Instance.new("UIListLayout", page) layout.Padding = UDim.new(0, 8) Pages[k] = page
end
Pages.Info.Visible = true Tabs.Info.TextColor3 = Color3.new(1,1,1)
...
local function CreateSection(parent, text)
    ...
    lbl.Size = UDim2.new(1, 0, 0, 30) lbl.BackgroundTransparency = 1 lbl.Text = "=== " .. text .. " ===" lbl.TextColor3 = Color3.fromRGB(0, 255, 255) lbl.Font = Enum.Font.GothamBold lbl.TextSize = 13 return lbl
end

local function CreateLabel(parent, text)
    local lbl = Instance.new("TextLabel", parent) lbl.Size = UDim2.new(1, 0, 0, 30) lbl.BackgroundTransparency = 1 lbl.Text = text lbl.TextColor3 = Color3.new(1,1,1) lbl.Font = Enum.Font.Gotham lbl.TextSize = 12 lbl.TextXAlignment = Enum.TextXAlignment.Left return lbl
end
...
local function CreateToggle(parent, name, defaultInput)
    ...
    local Label = Instance.new("TextLabel", Frame) Label.Size = UDim2.new(0.5, 0, 1, 0) Label.Position = UDim2.new(0.05, 0, 0, 0) Label.BackgroundTransparency = 1 Label.Text = name Label.TextColor3 = Color3.new(1,1,1) Label.Font = Enum.Font.Gotham Label.TextSize = 12 Label.TextXAlignment = Enum.TextXAlignment.Left
    local ToggleBtn = Instance.new("TextButton", Frame) ToggleBtn.Size = UDim2.new(0, 50, 0, 25) ToggleBtn.Position = UDim2.new(0.85, -10, 0.5, -12.5) ToggleBtn.BackgroundColor3 = Color3.fromRGB(255, 50, 50) ToggleBtn.Text = "OFF" ToggleBtn.TextColor3 = Color3.new(1,1,1) ToggleBtn.Font = Enum.Font.GothamBold Instance.new("UICorner", ToggleBtn)
    local InputBox = nil
    if defaultInput then
        InputBox = Instance.new("TextBox", Frame) InputBox.Size = UDim2.new(0, 40, 0, 25) InputBox.Position = UDim2.new(0.68, -15, 0.5, -12.5) InputBox.BackgroundColor3 = Color3.fromRGB(20, 20, 20) InputBox.TextColor3 = Color3.new(1,1,1) InputBox.Text = tostring(defaultInput) Instance.new("UICorner", InputBox)
    end
    return ToggleBtn, InputBox
end

local function CreateButton(parent, name, callback)
    local btn = Instance.new("TextButton", parent) btn.Size = UDim2.new(1, 0, 0, 40) btn.BackgroundColor3 = Color3.fromRGB(0, 150, 255) btn.Text = name btn.TextColor3 = Color3.new(1,1,1) btn.Font = Enum.Font.GothamBold btn.TextSize = 13 Instance.new("UICorner", btn) btn.MouseButton1Click:Connect(callback) return btn
end
...
task.spawn(function() pcall(function() local desc = Players:GetCharacterAppearanceAsync(LocalPlayer.UserId) local count = 0 for _,v in pairs(desc:GetChildren()) do if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") then count = count + 1 end end OutfitLabel.Text = "Trang phục đang mặc: " .. count .. " items" end) end)

local startTime = tick()
RunService.RenderStepped:Connect(function()
    local elapsed = tick() - startTime
    PlayTimeLabel.Text = string.format("Số phút chơi game: %02d:%02d:%02d", math.floor(elapsed / 3600), math.floor((elapsed % 3600) / 60), math.floor(elapsed % 60))
end)

-- ==========================================
-- HOME TAB
-- ==========================================
local wsBtn, wsBox = CreateToggle(Pages.Home, "Tốc Độ", 16) RegisterToggle(wsBtn, "WS")
local jpBtn, jpBox = CreateToggle(Pages.Home, "Nhảy (Jump)", 50) RegisterToggle(jpBtn, "JP")
local ncBtn, _ = CreateToggle(Pages.Home, "Xuyên Tường") RegisterToggle(ncBtn, "NC")
local acBtn, _ = CreateToggle(Pages.Home, "Auto Clicker")
local climbBtn, _ = CreateToggle(Pages.Home, "Bám Tường") RegisterToggle(climbBtn, "WallClimb")
local infJumpBtn, _ = CreateToggle(Pages.Home, "Infi Jump") RegisterToggle(infJumpBtn, "InfJump")
local godBtn, _ = CreateToggle(Pages.Home, "Godmode (Bất Tử)") RegisterToggle(godBtn, "Godmode")
local aJumpBtn, _ = CreateToggle(Pages.Home, "Auto Jumpwall") RegisterToggle(aJumpBtn, "AutoJumpwall")
local xyzBtn, _ = CreateToggle(Pages.Home, "Xem Tọa Độ (XYZ)") RegisterToggle(xyzBtn, "XYZ", function(v) XYZLabel.Visible = v end)
local glowBtn, _ = CreateToggle(Pages.Home, "Phát Sáng Toàn Thân") 
RegisterToggle(glowBtn, "FullBodyGlow", function(v) local char = LocalPlayer.Character if char then if v then local hl = Instance.new("Highlight", char) hl.Name = "SelfGlow" hl.FillColor = Color3.new(1,1,1) hl.OutlineColor = Color3.new(0,1,1) else if char:FindFirstChild("SelfGlow") then char.SelfGlow:Destroy() end end end end)
...
CreateButton(Pages.Home, "🎁 Lấy Tool Ẩn (Clone Item)", function()
    ...
    if foundTool then foundTool:Clone().Parent = LocalPlayer.Backpack end
end)
...
-- ESP & FLY TAB
-- ==========================================
local espBtn, _ = CreateToggle(Pages.ESP_Fly, "Định Vị (ESP)") RegisterToggle(espBtn, "ESP")
local healthEspBtn, _ = CreateToggle(Pages.ESP_Fly, "Bảng Điều Máu") RegisterToggle(healthEspBtn, "HealthESP")
local nameBtn, _ = CreateToggle(Pages.ESP_Fly, "Tên Hiển Thị") RegisterToggle(nameBtn, "NameESP")
local distBtn, _ = CreateToggle(Pages.ESP_Fly, "Khoảng Cách") RegisterToggle(distBtn, "Distance")
...
local function HandleFly(v, isV2, box)
    ...
    if v and char and char:FindFirstChild("HumanoidRootPart") then
        local hrp = char.HumanoidRootPart flyForce = Instance.new("BodyVelocity", hrp) flyForce.MaxForce = Vector3.new(9e9, 9e9, 9e9) flyForce.Velocity = Vector3.zero
        flyGyro = Instance.new("BodyGyro", hrp) flyGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9) flyGyro.P = 10000 char.Humanoid.PlatformStand = true
...
local followBtn, _ = CreateToggle(Pages.TP, "Theo Dõi Mục Tiêu") RegisterToggle(followBtn, "Follow")
...
-- TROLL & AURA TAB
-- ==========================================
local orbitBtn, _ = CreateToggle(Pages.Troll, "Xoay Quanh Người Chơi") RegisterToggle(orbitBtn, "Orbit")
...
local flingBtn, flingBox = CreateToggle(Pages.Troll, "Fling Aura", 50) RegisterToggle(flingBtn, "Fling")
local antikickBtn, _ = CreateToggle(Pages.Troll, "Anti-Kick") RegisterToggle(antikickBtn, "AntiKick")
local antiflingBtn, _ = CreateToggle(Pages.Troll, "Anti-Fling") RegisterToggle(antiflingBtn, "AntiFling")
local antivoidBtn, _ = CreateToggle(Pages.Troll, "Anti-Void") local spawnPoint = Vector3.new(0, 50, 0) RegisterToggle(antivoidBtn, "AntiVoid", function(v) if v and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then spawnPoint = LocalPlayer.Character.HumanoidRootPart.Position end end)

local rainbowSkinBtn, _ = CreateToggle(Pages.Aura, "Rainbow Skin") RegisterToggle(rainbowSkinBtn, "RainbowSkin")
local fireBtn, _ = CreateToggle(Pages.Aura, "Fire Aura") RegisterToggle(fireBtn, "Fire", function(v) local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") if v and hrp then local f = Instance.new("Fire", hrp) f.Name = "FAura" f.Size = 10 else if hrp and hrp:FindFirstChild("FAura") then hrp.FAura:Destroy() end end end)
local smokeBtn, _ = CreateToggle(Pages.Aura, "Smoke Aura") RegisterToggle(smokeBtn, "Smoke", function(v) local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") if v and hrp then local s = Instance.new("Smoke", hrp) s.Name = "SAura" s.Size = 8 else if hrp and hrp:FindFirstChild("SAura") then hrp.SAura:Destroy() end end end)
local lightBtn, _ = CreateToggle(Pages.Aura, "Lighting Aura") RegisterToggle(lightBtn, "Lighting", function(v) local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") if v and hrp then local l = Instance.new("Sparkles", hrp) l.Name = "LAura" else if hrp and hrp:FindFirstChild("LAura") then hrp.LAura:Destroy() end end end)
local trailBtn, _ = CreateToggle(Pages.Aura, "Đường Mòn (Trail)") local currentTrailColor, trailObj = Color3.new(1,0,0), nil
...
-- ==========================================
-- SKYBOX & ANIM TAB (ĐÃ SẮP XẾP LẠI)
-- ==========================================
CreateSection(Pages.Skybox, "SKYBOX CUSTOM")
CreateButton(Pages.Skybox, "🌌 Không Gian Vũ Trụ", function() local s = Instance.new("Sky", Lighting) s.SkyboxBk, s.SkyboxDn, s.SkyboxFt, s.SkyboxLf, s.SkyboxRt, s.SkyboxUp = "rbxassetid://153696504","rbxassetid://153696504","rbxassetid://153696504","rbxassetid://153696504","rbxassetid://153696504","rbxassetid://153696504" end)
CreateButton(Pages.Skybox, "✨ Hoàng Đạo (Zodiac)", function() local s = Instance.new("Sky", Lighting) s.SkyboxBk, s.SkyboxDn, s.SkyboxFt, s.SkyboxLf, s.SkyboxRt, s.SkyboxUp = "rbxassetid://160248248","rbxassetid://160248248","rbxassetid://160248248","rbxassetid://160248248","rbxassetid://160248248","rbxassetid://160248248" end)
CreateButton(Pages.Skybox, "🌙 Màn Đêm Tĩnh Lặng", function() local s = Instance.new("Sky", Lighting) s.SkyboxBk, s.SkyboxDn, s.SkyboxFt, s.SkyboxLf, s.SkyboxRt, s.SkyboxUp = "rbxassetid://143921677","rbxassetid://143921677","rbxassetid://143921677","rbxassetid://143921677","rbxassetid://143921677","rbxassetid://143921677" end)
...
CreateSection(Pages.Anim, "HOẠT ẢNH & NGOẠI HÌNH")
local zombiBtn, _ = CreateToggle(Pages.Anim, "Zombie Animation") RegisterToggle(zombiBtn, "Zombie", function(v) local a = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Animate") if v and a then if a:FindFirstChild("idle") then a.idle.Animation1.AnimationId="rbxassetid://313026115" end if a:FindFirstChild("walk") then a.walk.WalkAnim.AnimationId="rbxassetid://313032534" end end end)
local ninjaBtn, _ = CreateToggle(Pages.Anim, "Ninja Animation") RegisterToggle(ninjaBtn, "Ninja", function(v) local a = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Animate") if v and a then if a:FindFirstChild("idle") then a.idle.Animation1.AnimationId="rbxassetid://656117400" end if a:FindFirstChild("walk") then a.walk.WalkAnim.AnimationId="rbxassetid://656121766" end end end)
local ragdollBtn, _ = CreateToggle(Pages.Anim, "Ragdoll On Demand") RegisterToggle(ragdollBtn, "Ragdoll", function(v) if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.PlatformStand = v end end)
local bigHeadBtn, _ = CreateToggle(Pages.Anim, "Big Head") RegisterToggle(bigHeadBtn, "BigHead", function(v) local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid") if hum and hum:FindFirstChild("HeadScale") then hum.HeadScale.Value = v and 4 or 1 end end)
local noHeadFootBtn, _ = CreateToggle(Pages.Anim, "No Head & Foot") RegisterToggle(noHeadFootBtn, "NoHeadFoot", function(v) local c = LocalPlayer.Character if c then for _,p in pairs({"Head","RightFoot","RightLowerLeg","RightUpperLeg"}) do if c:FindFirstChild(p) then c[p].Transparency = v and 1 or 0 end end if c:FindFirstChild("Head") and c.Head:FindFirstChild("face") then c.Head.face.Transparency = v and 1 or 0 end end end)
local povBtn, _ = CreateToggle(Pages.Anim, "POV (Góc Nhìn 1)") RegisterToggle(povBtn, "POV", function(v) LocalPlayer.CameraMode = v and Enum.CameraMode.LockFirstPerson or Enum.CameraMode.Classic end)
local rnNameBtn, _ = CreateToggle(Pages.Anim, "Rainbow Name") RegisterToggle(rnNameBtn, "RainbowName")

CreateSection(Pages.Anim, "SHADER ĐỒ HỌA")
...
CreateButton(Pages.Anim, "⚡ Super Fast (Xóa Bóng)", function() ClearShaders() Lighting.GlobalShadows = false pcall(function() sethiddenproperty(Lighting, "Technology", Enum.Technology.Compatibility) end) for _, p in pairs(workspace:GetDescendants()) do if p:IsA("BasePart") then p.CastShadow = false p.Material = Enum.Material.SmoothPlastic end end end)
-- ==========================================
-- MM2 HACK TAB
-- ==========================================
local mm2ESPBtn, _ = CreateToggle(Pages.MM2, "Xác định Role (ESP)") RegisterToggle(mm2ESPBtn, "MM2_ESP", function(v) if not v then for _, plr in pairs(Players:GetPlayers()) do if plr.Character and plr.Character:FindFirstChild("MM2_HL") then plr.Character.MM2_HL:Destroy() end end end end)
local mm2AimBtn, _ = CreateToggle(Pages.MM2, "Aimbot (Diệt Sát Nhân)") RegisterToggle(mm2AimBtn, "MM2_Aim")
local mm2AutoCoinBtn, _ = CreateToggle(Pages.MM2, "Tự Động Nhặt Xu") RegisterToggle(mm2AutoCoinBtn, "MM2_Coin")
local mm2GunNotifBtn, _ = CreateToggle(Pages.MM2, "Gun Drop Notifier") RegisterToggle(mm2GunNotifBtn, "MM2_GunNotif")
local mm2GunTPBtn, _ = CreateToggle(Pages.MM2, "Teleport Đến Súng Rơi") RegisterToggle(mm2GunTPBtn, "MM2_GunTP")
...
workspace.ChildAdded:Connect(function(child)
    if child.Name == "GunDrop" then
        if states.MM2_GunNotif then SendNotification("⚠️ Cảnh Báo MM2", "Cảnh sát đã chết! Súng rơi kìa!") end
        if states.MM2_GunTP and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then LocalPlayer.Character.HumanoidRootPart.CFrame = child.CFrame end
    end
end)

-- ==========================================
-- WITH GAME? TAB (ĐÃ SẮP XẾP CHUẨN V9)
-- ==========================================
CreateSection(Pages.WithGame, "BLADE BALL / DEATH BALL")
...
local bbTrailBtn, _ = CreateToggle(Pages.WithGame, "Visual Trail / Đường Bay") RegisterToggle(bbTrailBtn, "BB_Trail")

CreateSection(Pages.WithGame, "HORROR - PETAPETA")
...
CreateSection(Pages.WithGame, "LEAF BLOWER SIMULATOR")
...
CreateSection(Pages.WithGame, "EVADE (TRỐN TRUY ĐUỔI)")
local evEspBtn, _ = CreateToggle(Pages.WithGame, "Nextbot ESP") RegisterToggle(evEspBtn, "EV_ESP")
local evStaminaBtn, _ = CreateToggle(Pages.WithGame, "Infinite Stamina") RegisterToggle(evStaminaBtn, "EV_Stamina")
local evReviveBtn, _ = CreateToggle(Pages.WithGame, "Auto Revive / Hồi Sinh") RegisterToggle(evReviveBtn, "EV_Revive")
local evSpeedBtn, evSpeedBox = CreateToggle(Pages.WithGame, "Tốc Độ Vượt Trội", 50) RegisterToggle(evSpeedBtn, "EV_Speed")
local evBrightBtn, _ = CreateToggle(Pages.WithGame, "Fullbright") RegisterToggle(evBrightBtn, "EV_Fullbright", function(v) Lighting.Ambient = v and Color3.new(1,1,1) or Color3.fromRGB(127,127,127) end)

CreateSection(Pages.WithGame, "STEAL AN EGG")
local seStealBtn, _ = CreateToggle(Pages.WithGame, "Auto Steal / Lấy Trứng") RegisterToggle(seStealBtn, "SE_Steal")
local seTreadBtn, _ = CreateToggle(Pages.WithGame, "Auto Farm Treadmill") RegisterToggle(seTreadBtn, "SE_Tread")
local seHatchBtn, _ = CreateToggle(Pages.WithGame, "Auto Hatch / Ấp Trứng") RegisterToggle(seHatchBtn, "SE_Hatch")
local seBaseBtn, _ = CreateToggle(Pages.WithGame, "Player Base ESP") RegisterToggle(seBaseBtn, "SE_BaseESP")
local seCoinBtn, _ = CreateToggle(Pages.WithGame, "Auto Collect Coins") RegisterToggle(seCoinBtn, "SE_Coin")
local seEggBtn, _ = CreateToggle(Pages.WithGame, "Egg ESP / Trứng Hiếm") RegisterToggle(seEggBtn, "SE_EggESP")
local seSellBtn, _ = CreateToggle(Pages.WithGame, "Auto Sell / Bán Pet") RegisterToggle(seSellBtn, "SE_Sell")
local seRerollBtn, _ = CreateToggle(Pages.WithGame, "Auto Re-roll") RegisterToggle(seRerollBtn, "SE_Reroll")
local seAntiBtn, _ = CreateToggle(Pages.WithGame, "Anti-Grab / Chống Cướp") RegisterToggle(seAntiBtn, "SE_Anti")
local seDailyBtn, _ = CreateToggle(Pages.WithGame, "Fast Claim Daily") RegisterToggle(seDailyBtn, "SE_Daily")
-- ==========================================
-- CÀI ĐẶT
-- ==========================================
...
local ESPColors = {{name="Xanh Lá",val=Color3.new(0,1,0)}, {name="Đỏ",val=Color3.new(1,0,0)}, {name="Trắng",val=Color3.new(1,1,1)}, {name="RGB (Động)",val="Anim_RGB"}, {name="Nhịp Đỏ",val="Anim_PulseRed"}, {name="Nhịp Xanh",val="Anim_PulseBlue"}, {name="Nhịp Vàng",val="Anim_PulseYellow"}, {name="Đa Sắc",val="Anim_Flash"}}
local TrailColors = {{name="Đỏ",val=Color3.new(1,0,0)}, {name="Trắng",val=Color3.new(1,1,1)}, {name="RGB (Động)",val="Anim_RGB"}, {name="Lửa Giao Thoa",val="Anim_Fire"}, {name="Biển Xanh",val="Anim_Ocean"}, {name="Neon Nhấp Nháy",val="Anim_Neon"}}
...
Pages.Info.CanvasSize = UDim2.new(0,0,0,200) Pages.Home.CanvasSize = UDim2.new(0,0,0,600) Pages.ESP_Fly.CanvasSize = UDim2.new(0,0,0,350) Pages.TP.CanvasSize = UDim2.new(0,0,0,300) Pages.Troll.CanvasSize = UDim2.new(0,0,0,350) Pages.Aura.CanvasSize = UDim2.new(0,0,0,350) Pages.Skybox.CanvasSize = UDim2.new(0,0,0,250) Pages.Anim.CanvasSize = UDim2.new(0,0,0,500) Pages.MM2.CanvasSize = UDim2.new(0,0,0,250) Pages.WithGame.CanvasSize = UDim2.new(0,0,0,1300) Pages.Set.CanvasSize = UDim2.new(0,0,0,250)

-- ==========================================
-- VÒNG LẶP HỆ THỐNG MASTER (V9)
-- ==========================================
local SpinForce
RunService.RenderStepped:Connect(function()
    local t = tick()
    ...
    if states.WS and hum and not states.EV_Speed then hum.WalkSpeed = tonumber(wsBox.Text) or 16 end
    ...
    if states.Follow and followTarget and followTarget.Character and followTarget.Character:FindFirstChild("HumanoidRootPart") and hrp then hrp.CFrame = followTarget.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 3) end
    if states.Orbit and orbitTarget and orbitTarget.Character and orbitTarget.Character:FindFirstChild("HumanoidRootPart") and hrp then hrp.CFrame = orbitTarget.Character.HumanoidRootPart.CFrame * CFrame.Angles(0, t * 5, 0) * CFrame.new(0, 0, 10) end
    ...
    -- Fix Rainbow Name (Kiểm tra liên tục)
    if states.RainbowName then
        if char:FindFirstChild("Head") then
            local rn = char.Head:FindFirstChild("RainbowNameTagV9")
            if not rn then rn = Instance.new("BillboardGui", char.Head) rn.Name = "RainbowNameTagV9" rn.Size = UDim2.new(0,200,0,50) rn.StudsOffset = Vector3.new(0,2.5,0) rn.AlwaysOnTop = true local rntxt = Instance.new("TextLabel", rn) rntxt.Size = UDim2.new(1,0,1,0) rntxt.BackgroundTransparency = 1 rntxt.Text = LocalPlayer.Name rntxt.Font = Enum.Font.GothamBold rntxt.TextScaled = true end
            rn:FindFirstChildOfClass("TextLabel").TextColor3 = cRGB
        end
    else
        if char:FindFirstChild("Head") and char.Head:FindFirstChild("RainbowNameTagV9") then char.Head.RainbowNameTagV9:Destroy() end
    end
    ...
    pcall(function()
        ...
        -- EVADE
        if states.EV_Speed and hum then hum.WalkSpeed = tonumber(evSpeedBox.Text) or 50 end
        if states.EV_Stamina and LocalPlayer:GetAttribute("Stamina") then LocalPlayer:SetAttribute("Stamina", 100) end
        if states.EV_Revive then for _, prompt in pairs(workspace:GetDescendants()) do if prompt:IsA("ProximityPrompt") and prompt.ActionText:lower():match("revive") then fireproximityprompt(prompt) end end end
        
        if states.EV_ESP or states.PP_ESP then
            for _, v in pairs(workspace:GetDescendants()) do
                if (v.Name == "Nextbot" or v:FindFirstChild("HumanoidRootPart") and not Players:GetPlayerFromCharacter(v)) then
                    if states.EV_ESP or states.PP_ESP then
                        local hl = v:FindFirstChild("BotESP") if not hl then hl = Instance.new("Highlight", v) hl.Name = "BotESP" hl.FillColor = Color3.new(1,0,0) hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop end
                        local bbg = v:FindFirstChild("BotText") if not bbg then bbg = Instance.new("BillboardGui", v) bbg.Name = "BotText" bbg.Size = UDim2.new(0,100,0,40) bbg.AlwaysOnTop = true bbg.StudsOffset = Vector3.new(0,3,0) local t = Instance.new("TextLabel", bbg) t.Size = UDim2.new(1,0,1,0) t.BackgroundTransparency=1 t.TextColor3=Color3.new(1,0,0) t.TextScaled=true t.Font=Enum.Font.GothamBold end
                        if hrp and v:FindFirstChild("HumanoidRootPart") then bbg.TextLabel.Text = v.Name .. "\n" .. math.floor((hrp.Position - v.HumanoidRootPart.Position).Magnitude) .. " M" end
                    else
                        if v:FindFirstChild("BotESP") then v.BotESP:Destroy() end if v:FindFirstChild("BotText") then v.BotText:Destroy() end
                    end
                end
            end
        end

        -- Horror (PetaPeta) Generic
        ...
        if states.PP_ItemESP then
            for _, v in pairs(workspace:GetDescendants()) do
                if v:IsA("Part") and (v.Name:lower():match("key") or v.Name:lower():match("ofuda") or v.Name:lower():match("box")) then
                    local hl = v:FindFirstChild("ItemESP") if not hl then hl = Instance.new("Highlight", v) hl.Name = "ItemESP" hl.FillColor = Color3.new(1,1,0) hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop end
                end
            end
        end
            ...
            if ballsFolder then
                for _, ball in pairs(ballsFolder:GetChildren()) do
                    if ball:IsA("BasePart") then
                        if states.BB_Trail and not ball:FindFirstChild("BBTrail") then local t = Instance.new("Trail", ball) t.Name = "BBTrail" t.Color = ColorSequence.new(cRGB) local a0 = Instance.new("Attachment", ball) a0.Position = Vector3.new(0,0.5,0) local a1 = Instance.new("Attachment", ball) a1.Position = Vector3.new(0,-0.5,0) t.Attachment0 = a0 t.Attachment1 = a1 end
        ...
        if (states.LBS_Clean or states.LBS_Cap or states.LBS_Sell or states.LBS_Speed) and hrp then
            ...
            if states.LBS_Clean then for _,v in pairs(workspace:GetDescendants()) do if v.Name == "Leaf" and v:IsA("BasePart") then firetouchinterest(hrp, v, 0) firetouchinterest(hrp, v, 1) end end end
        end

        -- Steal an Egg (Generic Logic)
        if states.SE_Steal and hrp then for _,v in pairs(workspace:GetDescendants()) do if v.Name:lower():match("egg") and v:IsA("BasePart") then firetouchinterest(hrp, v, 0) firetouchinterest(hrp, v, 1) end end end
        if states.SE_Tread and hrp then for _,v in pairs(workspace:GetDescendants()) do if v.Name:lower():match("treadmill") then hrp.CFrame = v.CFrame break end end end
        if states.SE_Coin then for _,v in pairs(workspace:GetDescendants()) do if v.Name:lower():match("coin") and v:IsA("BasePart") then firetouchinterest(hrp, v, 0) firetouchinterest(hrp, v, 1) end end end
        if states.SE_EggESP then
            for _,v in pairs(workspace:GetDescendants()) do
                if v.Name:lower():match("egg") and v:IsA("BasePart") then
                    local hl = v:FindFirstChild("SeEggESP") if not hl then hl = Instance.new("Highlight", v) hl.Name = "SeEggESP" hl.FillColor = Color3.new(1,1,0) hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop end
                end
            end
        end
        if states.SE_BaseESP then
            for _,v in pairs(workspace:GetDescendants()) do
                if v.Name:lower():match("base") and v:IsA("Model") then
                    local hl = v:FindFirstChild("SeBaseESP") if not hl then hl = Instance.new("Highlight", v) hl.Name = "SeBaseESP" hl.FillColor = Color3.new(0,1,1) hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop end
                end
            end
        end
    end)

    -- ESP NGƯỜI CHƠI CHUNG
    for _, plr in pairs(Players:GetPlayers()) do
