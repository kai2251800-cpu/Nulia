-- [[ LUNASPACE HUB - FULL OPTIONS & FIXED FLY ]] --
...
local guiName = "LunaSpaceHub_Pro"
...
-- ==========================================
-- 1. HỆ THỐNG MÀU CẦU VỒNG ĐỘNG (DYNAMIC RAINBOW)
-- ==========================================
...
-- Hiệu ứng phản chiếu nền cutscene
...
-- Thanh Loading Bar ngoài Cutscene
...
tab2Btn.MouseButton1Click:Connect(function() page1.Visible = false page2.Visible = true end)
...
-- ==========================================
-- 4. TẠO CÁC TIỆN ÍCH GIAO DIỆN (TOGGLE & INPUT)
-- ==========================================
...
-- ==========================================
-- 5. LẬP TRÌNH TÍNH NĂNG (CHỨC NĂNG CHÍNH)
-- ==========================================
...
local espHighlights = {}
createToggle(page1, "Định vị (ESP Xanh lá)", function(state)
    if state then
...
-- 6. Fly Siêu Mượt (Có bảng điều khiển ⬆️ ⬇️ và tốc độ)
local flySpeed = 16
...
local function toggleFly(state)
    ...
    if state then
        ...
        -- Tạo bảng điều khiển ⬆️ ⬇️ trên màn hình cho di động
        ...
        btnUp.Position = UDim2.new(0.5, -25, 0, 5)
        btnUp.Text = "⬆️"
        btnUp.TextSize = 22
        btnUp.BackgroundColor3 = Color3.fromRGB(0, 170, 127)
        btnUp.Parent = flyControlGui
        local buc = Instance.new("UICorner") buc.CornerRadius = UDim.new(1, 0) buc.Parent = btnUp

        local btnDown = Instance.new("TextButton")
        btnDown.Size = UDim2.new(0, 50, 0, 50)
        btnDown.Position = UDim2.new(0.5, -25, 0, 55)
        btnDown.Text = "⬇️"
        btnDown.TextSize = 22
        btnDown.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
        btnDown.Parent = flyControlGui
        local bdc = Instance.new("UICorner") bdc.CornerRadius = UDim.new(1, 0) bdc.Parent = btnDown

        local isUp, isDown = false, false
        btnUp.MouseButton1Down:Connect(function() isUp = true end)
        btnUp.MouseButton1Up:Connect(function() isUp = false end)
        btnDown.MouseButton1Down:Connect(function() isDown = true end)
        btnDown.MouseButton1Up:Connect(function() isDown = false end)
...
createInputToggle(page1, "Fly (Bay)", 16,
    function(state, val) flySpeed = val toggleFly(state) end,
    function(val) flySpeed = val end
)

-- 7. Khoảng cách (Distance M to Player)
local distBillboards = {}
...
-- ==========================================
-- 6. MỤC 2: TELEPORT PLAYER (CHỮ MÀU TRẮNG ĐỘNG)
-- ==========================================
...
-- ==========================================
-- 7. CHẠY LUỒNG INTRO CUTSCENE HOÀN HẢO
-- ==========================================
task.spawn(function()
    task.wait(1)
    loadingText.Text = "Đang kích hoạt..."
    
    -- Chạy thanh loading từ 0% đến 100% cực mượt
    for i = 0, 100, 2 do
    ...
    sendNotify("Kích hoạt LunaSpace Hub thành công! 🌙")
end)
