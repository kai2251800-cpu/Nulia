...
if CoreGui:FindFirstChild("FasterHubV7") then
    CoreGui:FindFirstChild("FasterHubV7"):Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "FasterHubV7"
...
-- ==========================================
-- 1. GIAO DIỆN KHỞI ĐỘNG (TỐI ƯU MOBILE)
-- ==========================================
...
BlockScreen.BackgroundTransparency = 0
...
task.spawn(function()
    pcall(function()
        StarterGui:SetCore("SendNotification", { Title = "Faster Hub V7", Text = "Bạn có muốn kích hoạt script không?", Duration = 5, Button1 = "Có✅", Button2 = "Không ❌", Callback = bindable })
    end)
end)
...
Title.Text = "FASTER HUB V7 [ULTRA VIP]"
...
TabScroll.CanvasSize = UDim2.new(3.5, 0, 0, 0)
...
local Tabs = {
    ...
    ComingSoon = CreateTabBtn("MỤC MỚI 😁")
}
...
CreateButton(Pages.Home, "🎁 Lấy Tool Ẩn (Clone Item)", function()
    ...
    if foundTool then
        ...
        SendNotification("Thành công", "Đã lấy vật phẩm vào balo!")
    else
        SendNotification("Thất bại", "Không tìm thấy Tool ẩn ở ReplicatedStorage.")
    end
end)
...
RegisterToggle(espBtn, "ESP") RegisterToggle(nameBtn, "NameESP") RegisterToggle(distBtn, "Distance")

local flyBtn, flyBox = CreateToggle(Pages.ESP_Fly, "Fly V1 (Cơ Bản)", 16)
local fly2Btn, fly2Box = CreateToggle(Pages.ESP_Fly, "Fly V2 (Bay Xuyên Tường)", 16)
...
local function HandleFly(v, isV2, box)
    ...
    if v and char and char:FindFirstChild("HumanoidRootPart") then
        ...
        flyConn = RunService.RenderStepped:Connect(function()
            ...
            local speed = tonumber(box.Text) or 16
            local moveDir = char.Humanoid.MoveDirection local ySpeed = 0
...
RegisterToggle(flyBtn, "Fly", function(v) HandleFly(v, false, flyBox) end)
RegisterToggle(fly2Btn, "FlyV2", function(v) HandleFly(v, true, fly2Box) end)
...
TeleOldIcon.MouseButton1Click:Connect(function()
    local char = LocalPlayer.Character if char and char:FindFirstChild("HumanoidRootPart") then
        ...
        if spawnLoc then char.HumanoidRootPart.CFrame = spawnLoc.CFrame + Vector3.new(0, 5, 0) elseif initialSpawnCFrame then char.HumanoidRootPart.CFrame = initialSpawnCFrame end
    end
end)
...
local oldIndex oldIndex = hookmetamethod(game, "__index", function(self, key)
    if not checkcaller() and states.MM2_Aim and self == LocalPlayer:GetMouse() and (key == "Hit" or key == "Target") then
        for _, plr in pairs(Players:GetPlayers()) do
            ...
            if plr ~= LocalPlayer and ((bp and bp:FindFirstChild("Knife")) or (char and char:FindFirstChild("Knife"))) and char and char:FindFirstChild("HumanoidRootPart") then
                return key == "Hit" and char.HumanoidRootPart.CFrame or char.HumanoidRootPart
            end
        end
...
CreateButton(Pages.Shader, "🌑 Dark Shader", function() ClearShaders() Lighting.Ambient = Color3.new(0,0,0) Lighting.OutdoorAmbient = Color3.new(0,0,0) Lighting.GlobalShadows = true end)
CreateButton(Pages.Shader, "🌅 Bình Minh", function() ClearShaders() Lighting.ClockTime = 6.5 Lighting.GlobalShadows = true Lighting.OutdoorAmbient = Color3.fromRGB(150, 100, 50) Instance.new("SunRaysEffect", Lighting).Intensity = 0.3 Instance.new("BloomEffect", Lighting).Intensity = 0.8 end)
CreateButton(Pages.Shader, "⚡ Super Fast (Tối Ưu Cực Độ)", function() ClearShaders() Lighting.GlobalShadows = false pcall(function() sethiddenproperty(Lighting, "Technology", Enum.Technology.Compatibility) end) for _, p in pairs(workspace:GetDescendants()) do if p:IsA("BasePart") then p.CastShadow = false p.Material = Enum.Material.SmoothPlastic end end end)
CreateButton(Pages.Shader, "🔄 Tắt Hết", function() ClearShaders() Lighting.ClockTime = 14 Lighting.Ambient = Color3.fromRGB(127,127,127) Lighting.OutdoorAmbient = Color3.fromRGB(127,127,127) end)

-- Cài đặt (Bảng màu lớn + Anim Colors)
...
CreateSelector(Pages.Set, "Màu Nền Hub", {{name="Đen",val=Color3.fromRGB(25,25,25)}, {name="Xanh",val=Color3.fromRGB(15,20,40)}}, function(v) MainFrame.BackgroundColor3 = v end)
...
local txtMsg = Instance.new("TextLabel", Pages.ComingSoon) txtMsg.Size = UDim2.new(1,0,0,100) txtMsg.BackgroundTransparency = 1 txtMsg.Text = "<<COMING SOON>>\nBạn muốn hỗ trợ game gì hãy comment ý kiến nhé 😁" txtMsg.TextColor3 = Color3.new(1,1,1) txtMsg.Font = Enum.Font.GothamBold txtMsg.TextScaled = true

Pages.Home.CanvasSize = UDim2.new(0, 0, 0, 520) Pages.ESP_Fly.CanvasSize = UDim2.new(0, 0, 0, 300) Pages.TP.CanvasSize = UDim2.new(0, 0, 0, 300) Pages.Troll.CanvasSize = UDim2.new(0, 0, 0, 350) Pages.MM2.CanvasSize = UDim2.new(0, 0, 0, 150) Pages.Aura.CanvasSize = UDim2.new(0, 0, 0, 350) Pages.Anim.CanvasSize = UDim2.new(0, 0, 0, 420) Pages.Shader.CanvasSize = UDim2.new(0, 0, 0, 250) Pages.Set.CanvasSize = UDim2.new(0, 0, 0, 350) Pages.ComingSoon.CanvasSize = UDim2.new(0,0,0,150)
-- ==========================================
-- VÒNG LẶP HỆ THỐNG
-- ==========================================
local SpinForce, NameTag
RunService.RenderStepped:Connect(function()
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("Head") then
            ...
            if states.ESP then
                if not hl then hl = Instance.new("Highlight", pChar) hl.Name = "ESP_HL" hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop end
                hl.FillColor = realESP
            else if hl then hl:Destroy() end end
            
            -- Tên Hiển Thị (NameESP) & Khoảng Cách (Distance) kết hợp vào 1 GUI
            local nameUI = pChar:FindFirstChild("NameDistUI")
            if states.NameESP or states.Distance then
                if not nameUI then
                    nameUI = Instance.new("BillboardGui", pChar) nameUI.Name = "NameDistUI" nameUI.Size = UDim2.new(0, 200, 0, 50) nameUI.StudsOffset = Vector3.new(0, 3, 0) nameUI.AlwaysOnTop = true
                ...
                if txt then
                    local finalTxt = ""
                    if states.NameESP then finalTxt = plr.Name end
                    ...
                    txt.Text = finalTxt
                    txt.TextColor3 = currentNameColor
                end
